...
// * Lectura de peso — solo si la bascula esta operativa
float findWeight() {
...
// Formato: {"capacity":1.50,"hoursSinceDrink":0,"totalDrunk":0.00,"bottlePlaced":true,"scaleOK":true}
void bluetoothSend(unsigned long timeSinceLastDrink) {
  ...
  String message = "{\"capacity\":"       + String(currentCapacity, 2) +
                   ...
                   ",\"scaleOK\":"         + String(scaleReady   ? "true" : "false") +
                   "}";
...
// * Inicializa la bascula con tare manual sin bloqueo (timeout 5s)
void initScale() {
  Serial.print("DT (A2): ");
  ...
  while (millis() - t0 < 5000UL) {
    if (scale.is_ready()) {
      long offset = scale.read();   // una sola lectura, sin bucle largo
      scale.set_offset(offset);
      scaleReady = true;
      Serial.print(F("Bascula OK — offset: "));
  ...
  Serial.println(F("AVISO: bascula no detectada, continuando sin ella"));
}
